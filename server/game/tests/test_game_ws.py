...
class GameConsumerTests(TransactionTestCase):
    def setUp(self):
        ...
        self.unjoined_user = User.objects.create_user(
            username="unjoined", password="password", email="unjoined@test.com"
        )
        self.user2 = User.objects.create_user(
            username="testuser2", password="password", email="testuser2@test.com"
        )
        ...
        SessionQuestion.objects.create(
            session=self.session, question=self.q1, order_index=0
        )
    ...
    async def test_connect_success(self):
        ...
        communicator = WebsocketCommunicator(
            self.application, f"/ws/game/{self.session.session_uuid}/", headers=headers
        )
    ...
    async def test_connect_unauthenticated(self):
        communicator = WebsocketCommunicator(
            self.application, f"/ws/game/{self.session.session_uuid}/"
        )
        connected, _ = await communicator.connect()
    ...
    async def test_receive_invalid_payload_format_returns_error(self):
        ...
        await communicator.send_json_to(
            {"action": GameAction.NOMINATE_PLAYER, "payload": {}}
        )
        response = await communicator.receive_json_from()
    ...
    async def test_submit_answer_rejects_answer_text_payload_field(self):
        ...
        await communicator.send_json_to(
            {"action": GameAction.SUBMIT_ANSWER, "payload": {"answer_text": "wrong"}}
        )
        response = await communicator.receive_json_from()
    ...
    async def test_action_domain_validation_error_returns_error_message(self):
        ...
        await communicator.send_json_to(
            {"action": GameAction.SUBMIT_ANSWER, "payload": {"answer": "yes"}}
        )
        response = await communicator.receive_json_from()
